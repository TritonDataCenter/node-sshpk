...
function read(buf, options) {
	...
	si += publicLines;
	if (lines[si]) {
		// See https://tartarus.org/~simon/putty-prerel-snapshots/htmldoc/AppendixC.html#ppk-v2
		parts = splitHeader(lines[si++]);
		...
		if (encryption === 'aes256-cbc') {
			var encryptionKey = _derivePPKv2EncryptionKey(options.passphrase)
			var decipher = crypto.createDecipheriv('aes-256-cbc', encryptionKey, Buffer.from(Array.from({length: 16}).map(_ => 0)));
...
function _derivePPKv2EncryptionKey(passphrase) {
