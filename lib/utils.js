...
function addRSAMissing(key) {
	...
	if (!key.part.dmodp) {
		...
		var hex = dmodp.toString(16);
		if (hex.length % 2 == 1)
			hex = '0' + hex;
		dmodp = new Buffer(hex, 'hex');
		dmodp = mpNormalize(dmodp);
	...
	if (!key.part.dmodq) {
		...
		var hex = dmodq.toString(16);
		if (hex.length % 2 == 1)
			hex = '0' + hex;
		dmodq = new Buffer(hex, 'hex');
		dmodq = mpNormalize(dmodq);
